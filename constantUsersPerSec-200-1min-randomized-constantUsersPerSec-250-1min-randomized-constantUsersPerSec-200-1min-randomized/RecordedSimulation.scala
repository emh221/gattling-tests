...
class RecordedSimulation extends Simulation {
    ...
    setUp(scn.inject(constantUsersPerSec(200) during (1 minute) randomized, constantUsersPerSec(250) during (1 minute) randomized, constantUsersPerSec(200) during (1 minutes) randomized )).protocols(httpProtocol)
